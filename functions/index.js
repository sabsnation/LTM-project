...
exports.sendManualUnlockEmail = onCall(async (request) => {
  ...
  if (!request.auth) {
    throw new Error('Você deve estar autenticado para chamar esta função.');
  }
  
  const { letterId, userId } = request.data;
  
  if (!letterId || !userId) {
    throw new Error('Os parâmetros letterId e userId são obrigatórios.');
  }
  ...
  try {
    ...
    if (letterData.author_id !== userId) {
      throw new Error('Você não tem permissão para acessar esta carta.');
    }
